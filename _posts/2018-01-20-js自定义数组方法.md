...
```js
Array.prototype.m_pop = function () {
...
Array.prototype.m_slice = function (start, end) {
	...
	start = indexHandler(this, start);
	end = indexHandler(this, end);
...
// 封装处理参数的方法
function indexHandler(arr, index) {
	if (index === undefined) {
		index = arr.length;
	} else {
	...
	//处理负数
	index = index >= 0 ? index : index + arr.length;
	//处理下限值

...
当获取对象属性名对应的属性值，首先会是否是私有属性，如果私有属性中不存在，那么对象会默认通过`__proto__`继续向上查找，如果上级也没有，那么会继续通过`__proto__`向上查找，直到找到`Object`的原型为止，如果`Object`中也不存在，那么会输出`undefined`.这样一级一级向上查找过程会形成**原型链**.
...
1. 把类 A 的实例的空间赋值给类 B 的原型，那么类 B 的实例不仅可以调用类 A 的公有属性，还可以调用类 A 的实例的私有属性；这种继承方式就是**原型继承**
...
2. call : 改变 this 的指向 A 方法在方法 B 中执行，改变类 A 中的 this 指向，让其指向类 B 中的 this（类 B 的实例）；类 B 创建的实例就具有类 A 的私有属性；
